...
if (!inputPath || !outputDir) {
  throw new Error("Uso: finalize-frozen-dividend-case <entrada.json> <diretório-saída>");
}
const input = JSON.parse(await readFile(inputPath, "utf8")) as SingleFrozenDividendInput;
const result = new SingleFrozenDividendCaseFinalizer().finalize(input);
...
console.log(JSON.stringify({
  ...
  observations: result.case.observations.length,
  exclusions: result.audit.exclusions.length,
}));
