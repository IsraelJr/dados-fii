...
async function requestJson(url: string, init?: RequestInit): Promise<CohortBacktestResponse> {
  ...
  return payload as CohortBacktestResponse;
}
...
export default function CohortBacktestPanel() {
  ...
  async function load(silent = false) {
    ...
    setError("");
    try {
      const response = await requestJson("/api/admin/system/risk-lab/cohort-backtest");
    ...
    try {
      ...
      setMessage(
        response.evidence?.status === "passed"
          ? "Todos os gates da Sprint 3.5 foram aprovados."
          : response.evidence?.status === "failed"
            ? "A execução terminou e registrou blockers objetivos para correção."
            : "A execução foi iniciada. O painel atualizará o status automaticamente.",
      );
    } catch (caught) {
  ...
  return (
    <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Um clique executa os seis fundos da coorte, verifica fontes primárias, look-ahead,
            cobertura, falsos positivos e falsos negativos. O botão não aprova conteúdo técnico
            e não altera Premium ou notificações.
          </p>
        </div>
      ...
      {error && (
        <p className="mt-4 rounded-2xl bg-red-50 p-4 text-sm font-bold text-red-900 ring-1 ring-red-200">
          {error}
        </p>
      )}
      {message && (
        <p className="mt-4 rounded-2xl bg-emerald-50 p-4 text-sm font-bold text-emerald-900 ring-1 ring-emerald-200">
          {message}
        </p>
      )}
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row">
        <button
          type="button"
          disabled={!enabled || loading || running || evidence?.status === "running"}
        ...
        >
          {running || evidence?.status === "running"
            ? <Loader2 className="animate-spin" size={18} />
            : <PlayCircle size={18} />}
          {running || evidence?.status === "running"
            ? "Executando e auditando…"
            : evidence?.status === "failed"
              ? "Executar novamente após correções"
              : "Executar pendências automaticamente"}
        </button>
        <button
          type="button"
      ...
      {loading && (
        <p className="mt-5 flex items-center gap-2 text-sm font-bold text-slate-500">
          <Loader2 className="animate-spin" size={17} /> Carregando status e evidências…
        </p>
      )}
      ...
      {!loading && metrics && (
        <div className="mt-6">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Small label="Início" value={dateTime(evidence?.startedAt)} />
          ...
          <table className="w-full min-w-[860px] text-left text-sm">
            <thead className="bg-slate-100 text-xs uppercase tracking-wide text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3">Fonte primária</th>
                <th className="px-4 py-3">Look-ahead</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {evidence.cases.map((item) => (
                <tr key={item.ticker}>
                  <td className="px-4 py-3 font-black text-slate-900">{item.ticker}</td>
                  <td className="px-4 py-3">{item.role}</td>
                  <td className="px-4 py-3">{item.outcome}</td>
                  <td className="px-4 py-3">{item.sourceCoveragePercent}%</td>
                  <td className="px-4 py-3">{item.primaryEvidenceComplete ? "Completa" : "Incompleta"}</td>
                  <td className="px-4 py-3">{item.lookAheadDetected ? "Detectado" : "Nenhum"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {!loading && evidence?.blockers.length ? (
        <div className="mt-6 rounded-2xl bg-red-50 p-5 text-red-950 ring-1 ring-red-200">
          ...
          <ul className="mt-3 space-y-2 text-sm leading-6">
            {evidence.blockers.map((blocker, index) => <li key={`${index}-${blocker}`}>• {blocker}</li>)}
          </ul>
        </div>
      ) : null}
    </section>
