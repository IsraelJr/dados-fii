...
function EventList({ title, events, emptyText }: { title: string; events: CalendarEvent[]; emptyText: string }) {
  return (
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 text-gray-100">
      ...
      {!loading && !error && (
        <div className="space-y-6">
          ...
          <EventList title="Anunciados no mês atual" events={currentMonth} emptyText="Nenhum comunicado do mês atual encontrado." />
          <EventList title="Pagos recentemente" events={paidRecently} emptyText="Nenhum pagamento recente encontrado." />
        </div>
      )}
    </main>
  );
