...
import { AlertTriangle, CalendarDays, Download, Loader2, Minus, Plus, RefreshCw, Save, Trash2, TrendingDown, TrendingUp } from "lucide-react";
...
function getDailyVariation(data: any) {
  const raw = data?.variation ?? data?.dailyVariation ?? data?.changePercent;
...
export default function WalletPage() {
  ...
  const insights = useMemo(() => {
    ...
    return {
      ...
      segmentBreakdown,
      mainSegment: segmentBreakdown[0],
    };
  }, [loaded]);
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <PageHeader title="Minha Carteira" subtitle="Salva neste navegador. Adicione seus FIIs e veja renda estimada, movimentação diária e próximos pagamentos." action={<Link href="/calendario-dividendos-fiis" className="rounded-full bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700">Calendário público</Link>} />
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <MetricBox label="Valor aproximado da carteira" value={formatCurrency(insights.currentValue)} detail="Calculado pelo preço atual retornado pela consulta." tone="indigo" />
      ...
      <section className="mt-6 rounded-2xl bg-gray-900 p-5 text-gray-100 shadow-lg ring-1 ring-white/10">
        ...
        {!items.length ? <p className="mt-3 text-sm font-medium text-gray-300">Adicione FIIs para gerar um resumo automático da carteira.</p> : (
      ...
      <section className="mt-6 rounded-2xl bg-gray-900 p-5 text-gray-100 shadow-lg ring-1 ring-white/10">
        <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-gray-300">{insights.waiting.length ? `${insights.waiting.length} FII(s) da carteira ainda não têm rendimento de ${MONTHS_PTBR[insights.currentMonth]} na base. Estimativa pendente: ${formatCurrency(insights.pendingIncome)}.` : `Todos os FIIs carregados já têm rendimento de ${MONTHS_PTBR[insights.currentMonth]} na base.`}</p>
          ...
          <button type="button" onClick={updateMissingDividends} disabled={!insights.waiting.length || updatingMissing} className="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-gray-700 disabled:text-gray-400"><RefreshCw size={16} className={updatingMissing ? "animate-spin" : ""} /> Atualizar pendentes</button>
        </div>
      </section>
      ...
      <section className="mt-6 rounded-2xl bg-gray-900 p-5 text-gray-100 shadow-lg ring-1 ring-white/10">
        <div className="mb-4 flex items-center justify-between"><h2 className="text-xl font-extrabold text-white">FIIs na carteira</h2>{loading && <span className="inline-flex items-center gap-2 text-sm font-medium text-gray-300"><Loader2 className="animate-spin" size={16} /> Atualizando...</span>}</div>
        {!items.length ? <p className="rounded-xl border border-dashed border-gray-700 p-6 text-center text-sm font-medium text-gray-300">Sua carteira ainda está vazia. Comece adicionando um ticker e a quantidade de cotas.</p> : (
          <>
      ...
      <section className="mt-6 rounded-2xl bg-gray-900 p-5 text-gray-100 shadow-lg ring-1 ring-white/10">
        ...
        {!displayedUpcomingPayments.length ? <p className="text-sm font-medium text-gray-300">Ainda não há pagamentos futuros identificados para os FIIs da sua carteira.</p> : (
          <ul className={`${shouldScrollUpcomingPayments ? "max-h-[520px] overflow-y-auto pr-2" : ""} space-y-3`}>{displayedUpcomingPayments.map((payment) => <li key={`${payment.ticker}-${payment.date}-${payment.month}`} className="flex flex-col justify-between gap-1 rounded-xl bg-gray-800 p-4 md:flex-row md:items-center"><div><FiiTickerLink ticker={payment.ticker} /><span className="ml-2 text-sm font-medium text-gray-300">{MONTHS_PTBR[payment.month] || payment.month} · Pagamento em {payment.date}</span></div><strong className="text-green-300">{formatCurrency(payment.amount)}</strong></li>)}</ul>
        )}
      </section>
    </main>
  );
...
function DailyVariationBadge({ value }: { value?: number }) {
  ...
  if (!Number.isFinite(variation) || Math.abs(variation) < 0.005) {
    return <span className="inline-flex items-center gap-1 rounded-full bg-gray-800 px-2 py-0.5 text-xs font-extrabold text-gray-400 ring-1 ring-gray-700" title="Sem variação no dia"><Minus size={12} /> -</span>;
  }
...
function WalletMobileCard({ item, nextPayment, draftQuotas, changed, onQuotaChange, onSave, onRemove }: { item: any; nextPayment?: Payment; draftQuotas: string; changed: boolean; onQuotaChange: (value: string) => void; onSave: () => void; onRemove: () => void }) {
  return (
    <article className="rounded-2xl bg-gray-800 p-4 text-gray-100 ring-1 ring-white/10">
      ...
      <div className="grid gap-3"><InfoRow label="Preço atual" value={item.data?.price || "-"} /><InfoRow label="Último rendimento" value={item.lastDividend ? `${MONTHS_PTBR[item.lastDividend.month] || item.lastDividend.month}: ${item.lastDividend.info.earnings}` : item.error || "-"} /><InfoRow label="Anunciado no mês" value={item.currentDividend ? item.currentDividend.info.earnings : "Aguardando"} /><InfoRow label="Renda estimada" value={formatCurrency(item.estimatedIncome)} highlight="green" /><InfoRow label="Próximo pagamento" value={formatPaymentSummary(nextPayment)} /></div>
      <div className="mt-4 grid grid-cols-[1fr_auto] gap-2"><input value={draftQuotas} onChange={(event) => onQuotaChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onSave(); }} inputMode="decimal" className="rounded-lg border border-gray-700 bg-gray-950 p-2 text-white outline-none focus:border-indigo-400" /><button onClick={onSave} disabled={!changed} className={`inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 font-bold ${changed ? "bg-indigo-600 text-white hover:bg-indigo-700" : "cursor-not-allowed bg-gray-700 text-gray-400"}`}><Save size={16} /> Salvar</button></div>
    </article>
  );
}
...
function RankingCard({ title, items }: { title: string; items: Array<{ ticker: string; value: string }> }) {
  ...
  return (
    <div className="rounded-2xl bg-gray-900 p-5 text-gray-100 shadow-lg ring-1 ring-white/10">
      ...
      {!items.length ? <p className="text-sm font-medium text-gray-300">Sem dados ainda.</p> : isSegmentDistribution ? (
        <ol className="space-y-4 text-sm">{items.map((item, index) => <li key={`${title}-${item.ticker}`} className="rounded-lg bg-gray-800 px-3 py-3 text-gray-200"><div className="mb-2 flex items-center justify-between gap-3"><span className="font-medium"><strong className="text-gray-400">#{index + 1}</strong> {item.ticker}</span><strong className="text-indigo-200">{item.value}</strong></div><div className="h-2.5 overflow-hidden rounded-full bg-gray-700"><div className={`h-full rounded-full bg-indigo-400 ${getBarWidthClass(item.value)}`} /></div></li>)}</ol>
      ) : (
        <ol className="space-y-2 text-sm">{items.map((item, index) => <li key={`${title}-${item.ticker}`} className="flex justify-between gap-3 rounded-lg bg-gray-800 px-3 py-2 text-gray-200"><span><strong className="text-gray-400">#{index + 1}</strong> <FiiTickerLink ticker={item.ticker} /></span><strong className="text-indigo-200">{item.value}</strong></li>)}</ol>
      )}
    </div>
  );
}
