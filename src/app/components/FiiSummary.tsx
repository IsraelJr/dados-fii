...
import FiiRegulatoryReport from "./FiiRegulatoryReport";
...
export default function FiiSummary({
    ...
    const earningsYearData = data?.[`earnings${currentYear}`] || data?.[`earnings${currentYear - 1}`] || {};
    const dividends = getCurrentYearDividends(earningsYearData);
    const numericPrice = typeof data?.price === "string"
        ? parseFloat(data.price.replace("R$ ", "").replace(",", "."))
        : Number(data?.price);

    return (
        <div className="mx-auto w-full max-w-3xl">
            <div className="fii-summary mt-8 overflow-hidden rounded-2xl bg-gray-900 p-4 text-gray-100 shadow-lg md:p-6">
                <div className="fii-summary-grid grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <div className="fii-summary-card fii-summary-card-full min-w-0 overflow-hidden rounded-xl bg-gray-800 p-4 md:col-span-2 lg:col-span-3">
                        <div className="flex min-w-0 flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            ...
                            <div className="shrink-0"><FiiAlert fiiCode={data.code} /></div>
                        </div>
                    </div>
                    ...
                    <SummaryCard
                        ...
                        label="Ticker"
                        value={<Link href={`/fii/${data.code}`} className="font-bold text-indigo-200 hover:text-indigo-100">{data.code}</Link>}
                    />
                    <SummaryCard icon={data.active ? <CheckCircle className="text-green-400" /> : <XCircle className="text-red-400" />} label="Ativo" value={data.active ? "Sim" : "Não"} />
                    <SummaryCard icon={data.isIFIX ? <CheckCircle className="text-green-400" /> : <XCircle className="text-red-400" />} label="IFIX" value={data.isIFIX ? "Sim" : "Não"} glossaryHref="/glossario#ifix" />
                    <SummaryCard icon={<DollarSign className="text-green-400" />} label="Preço" value={data.price || "N/A"} glossaryHref="/glossario#preco-cota" />
                    <SummaryCard icon={<Hash className="text-orange-400" />} label="Total de cotas" value={data.numberShares?.toLocaleString("pt-BR") || "N/A"} glossaryHref="/glossario#cotas" />
                    <SummaryCard icon={<Building2 className="text-purple-400" />} label="Segmento" value={data.segment_new || data.segment || "N/A"} glossaryHref="/glossario#segmentos" />
                </div>
                ...
                <div className="mt-4 rounded-xl bg-gray-800/80 p-4 text-xs leading-5 text-gray-300 ring-1 ring-white/5">
                    ...
                    <Link href="/fontes-dos-dados" className="mt-2 inline-flex font-bold text-indigo-300 hover:text-indigo-100">Entender fontes e limitações</Link>
                </div>

                {data?.code && <AddToWalletButton ticker={data.code} />}

                <h3 className="mb-2 mt-6 flex items-center gap-2 text-xl font-bold">
                    💰 Dividendos ({earningsYearData === data?.[`earnings${currentYear}`] ? currentYear : currentYear - 1}){" "}
                    <GlossaryLink href="/glossario#dividendos-dy" />
                </h3>

                <div className="rounded-xl bg-gray-800 p-4">
                    {dividends.length === 0 ? (
                        <p className="text-center text-gray-400">Sem dados de dividendos para exibir.</p>
                    ) : (
                    ...
                    {isCurrentMonthMissing && data?.code && <UpdateDividendButton ticker={data.code} onSuccess={onDividendUpdate} />}
                </div>

                <h3 className="mb-2 mt-6 flex items-center justify-center gap-2 text-center text-xl font-bold">
                    <TrendingUp size={24} className="text-indigo-400" /> Planejamento Financeiro
                </h3>
                <SimulationCard
            ...
            {data?.code && <FiiRegulatoryReport ticker={data.code} />}
        </div>
    );
}
