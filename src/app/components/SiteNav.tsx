      ...
      <nav className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
        <Link href="/" className="flex items-center gap-2 font-extrabold text-slate-900" aria-label="Dados FII - Início">
          ...
          <span className="leading-tight">
            Dados FII
            <span className="block text-xs font-bold text-slate-500">Fundos imobiliários</span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-1 overflow-x-auto rounded-full bg-slate-100 p-1">
          {links.map(({ href, label, icon: Icon }) => (
            <Link
              key={href}
              href={href}
              className="inline-flex shrink-0 items-center gap-2 rounded-full px-3 py-2 text-xs font-extrabold text-slate-700 hover:bg-white hover:text-indigo-700 hover:shadow-sm md:text-sm"
            >
              <Icon size={15} /> {label}
            </Link>
          ))}
