...
import type { PremiumFundReport, PremiumRecommendation, PremiumScenario, PremiumStressCase } from "../../types/premium-report";
...
export const PREMIUM_REPORT_VERSION = "1.0.0";
...
function stressTest(report: FreeFundReport): PremiumStressCase[] {
  ...
  const cases = [
    { id: "mild" as const, label: "Estresse leve", priceShockPercent: -10, dividendShockPercent: -10, riskPenalty: 2 },
    { id: "moderate" as const, label: "Estresse moderado", priceShockPercent: -20, dividendShockPercent: -15, riskPenalty: 5 },
    { id: "severe" as const, label: "Estresse severo", priceShockPercent: -30, dividendShockPercent: -25, riskPenalty: 10 },
  ];
...
function scenarios(report: FreeFundReport): PremiumScenario[] {
  ...
  const dividend = report.market.lastDividend;
  const definitions = [
    { id: "positive" as const, label: "Sensibilidade positiva", price: 10, dividend: 5, assumptions: ["Cotação +10%", "Rendimento mensal +5%", "Sem mudança estrutural adicional"] },
    { id: "base" as const, label: "Base estática", price: 0, dividend: 0, assumptions: ["Cotação constante", "Rendimento mensal constante", "Manutenção dos dados atuais"] },
    { id: "adverse" as const, label: "Sensibilidade adversa", price: -15, dividend: -20, assumptions: ["Cotação -15%", "Rendimento mensal -20%", "Sem recuperação automática presumida"] },
  ];
...
function comparablePeers(report: FreeFundReport, funds: PublicFundData[]) {
  ...
  const sameSegment = funds.filter((fund) => fund.ticker !== report.ticker
    ...
    && fund.scores);
  const sameKind = funds.filter((fund) => fund.ticker !== report.ticker && fund.fundKind === report.identity.fundKind && fund.scores);
...
function comparative(report: FreeFundReport, funds: PublicFundData[]) {
  ...
  for (const key of SCORE_KEYS) {
    current[key] = report.scores?.[key].score ?? null;
    const values = peers.map((peer) => peer.scores?.[key].score).filter((value): value is number => typeof value === "number");
  ...
  const percentile = currentPremium === null || !peers.length
    ? null
...
function recommendations(report: FreeFundReport, valuationResult: ReturnType<typeof valuation>): PremiumRecommendation[] {
  ...
  if (scores) {
    ...
    for (const key of SCORE_KEYS.filter((item) => item !== "premium")) {
      const score = scores[key];
      if (score.score < 45 || score.confidence < 35) {
        items.push({
          priority: score.score < 35 ? "high" : "medium",
          category: labels[key],
          action: `Acompanhar e completar evidências de ${labels[key]} antes de aumentar a exposição.`,
...
export class PremiumReportEngine {
  generate(freeReport: FreeFundReport, peers: PublicFundData[], aiAnalysis: FundAIInsights, generatedAt = new Date().toISOString()): PremiumFundReport {
    ...
    return {
      ...
      stressTest: stressTest(freeReport),
      scenarios: scenarios(freeReport),
