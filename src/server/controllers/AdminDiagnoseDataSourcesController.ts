...
async function getLatestBenchmark() {
  try {
    ...
    return {
      ok: false,
      error: "Erro ao consultar MarketBenchmarks/latest",
    };
  }
}
...
export async function GET(req: NextRequest) {
  try {
    const authorization = await requireAdminOrCron(req, { scope: "diagnose-data-sources" });
    ...
    return NextResponse.json({ ok: false, error: "Erro ao diagnosticar fontes de dados." }, { status: 500 });
  }
}
