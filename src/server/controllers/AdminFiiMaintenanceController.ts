...
export async function POST(req: NextRequest) {
    try {
        const authorization = await requireAdminOrCron(req, { scope: "fii-maintenance" });
        ...
        for (const doc of docs) {
            ...
            try {
                ...
                results.push({ ticker, documentId: doc.id, ok: false, error: "Falha interna na manutenção deste fundo." });
            }
        }
        ...
        return NextResponse.json({ error: "Erro na manutenção temporária." }, { status: 500 });
    }
}
