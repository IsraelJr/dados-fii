...
async function getStats() {
  try {
    ...
    return { ok: false, visits: 0, searches: 0, error: "Erro ao consultar estatisticas" };
  }
}
...
async function getBenchmarks() {
  try {
    ...
    return { ok: false, error: "Erro ao consultar benchmarks" };
  }
}
...
async function getReports() {
  try {
    ...
    return { ok: false, totalSample: 0, done: 0, pending: 0, failed: 0, error: "Erro ao consultar relatorios" };
  }
}
...
async function getPortfolioNotifications() {
  try {
    ...
    return {
      ...
      digestsSent: 0,
      recent: [],
      error: "Erro ao consultar notificações da carteira",
    };
  }
}
...
async function getEvents() {
  try {
    ...
    return [{ id: "observability-error", type: "system", ok: false, statusCode: 500, ticker: null, tickers: [], message: null, error: "Erro ao consultar eventos", source: "observability", createdAt: new Date().toISOString() }];
  }
}
