...
export async function POST(req: NextRequest) {
  try {
    const authorization = await requireAdminOrCron(req, { scope: "update-dividends-batch" });
    ...
    for (const doc of docs) {
      ...
      try {
        ...
        results.push({ ok: false, ticker, documentId: doc.id, error: "Falha interna na atualização deste fundo." });
      }
    }
    ...
    return NextResponse.json({ error: "Erro ao atualizar dividendos em lote." }, { status: 500 });
  }
}
