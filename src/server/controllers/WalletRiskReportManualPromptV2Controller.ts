...
export async function POST(req: Request) {
  try {
    ...
    const [benchmarkData, snapshot] = await Promise.all([
      getMarketBenchmarks().catch((err) => ({ unavailable: true, error: "Benchmarks indisponíveis" })),
    ...
    return NextResponse.json({ ok: false, error: "Erro ao gerar prompt manual do relatório." }, { status: err.status || 500 });
  }
}
