...
const sessionStore = read("src/server/auth/WalletSessionStore.ts");
...
test("sessão curta é revogável, isolada e rejeita expiração", () => {
  ...
  assert.match(identityResolver, /walletSessionStore\.verify/);
  assert.match(firebaseSession, /walletSessionStore\.issueFirebaseSession/);
  assert.match(firebaseSession, /walletSessionStore\.revokeFamily/);
  assert.match(sessionStore, /runTransaction/);
  assert.match(sessionStore, /currentGeneration/);
  assert.match(sessionStore, /revokedBeforeGeneration/);
  assert.match(sessionPolicy, /walletSessionFamilyMatches/);
  assert.doesNotMatch(sessionStore, /isVip|isPremium|entitlement|\bplan\b/);
