...
test("automatic API is admin-only, rate-limited and ticker-only", async () => {
  ...
  assert.doesNotMatch(code, /action === "approve"|action === "reject"|documentId/);
});

test("automatic page asks only for a ticker and never requests document validation", async () => {
  ...
  assert.doesNotMatch(code, /documentId|candidateId|Aprovar|Rejeitar|checkbox|confirmed/);
});
